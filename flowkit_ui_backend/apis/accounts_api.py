...
async def delete_user(
    ...
    token_auth0: TokenModel = Security(
        get_token_auth0, scopes=["profile"]
    ),
    request: Request = None
) -> None:
    """Deletes a single user"""

    if not hasattr(accounts_api_impl, 'delete_user') or not callable(getattr(accounts_api_impl, 'delete_user')):
        raise NotImplementedError("The /delete_user endpoint is not yet implemented")

    try:
        logger.debug("Starting request")
        impl_result = await accounts_api_impl.delete_user(uid, pool=request.app.state.pool, token_model=token_auth0)
...
@router.get(
    "/users/{uid}",
    responses={
        200: {"model": UserMetadata, "description": "OK: The user exists and their metadata is returned in the content part of this response."},
        400: {"description": "Bad Request: The request is malformed, incomplete or otherwise invalid."},
        401: {"description": "Unauthorized: The user does not have the permissions to access this resource."},
        404: {"description": "Not Found: The requested data could not be found."},
        429: {"description": "Too Many Requests: The user has exceeded the limit of allowed simultaneous requests."},
...
async def get_user(
    ...
    if not hasattr(accounts_api_impl, 'get_user') or not callable(getattr(accounts_api_impl, 'get_user')):
        raise NotImplementedError("The /get_user endpoint is not yet implemented")

    try:
        logger.debug("Starting request")
        impl_result = await accounts_api_impl.get_user(uid, pool=request.app.state.pool, token_model=token_auth0)
        logger.debug("Request ready")
        content = impl_result[0] if isinstance(impl_result, tuple) else impl_result
        if isinstance(impl_result, Response):
            return impl_result
...
@router.get(
    ...
    responses={
        204: {"description": "OK: The password reset has been initiated and the suer should check their email for further instructions."},
        400: {"description": "Bad Request: The request is malformed, incomplete or otherwise invalid."},
        401: {"description": "Unauthorized: The user does not have the permissions to access this resource."},
        404: {"description": "Not Found: The requested data could not be found."},
        429: {"description": "Too Many Requests: The user has exceeded the limit of allowed simultaneous requests."},
...
async def reset_password(
    ...
    if not hasattr(accounts_api_impl, 'reset_password') or not callable(getattr(accounts_api_impl, 'reset_password')):
        raise NotImplementedError("The /reset_password endpoint is not yet implemented")

    try:
        logger.debug("Starting request")
        impl_result = await accounts_api_impl.reset_password(email, pool=request.app.state.pool, token_model=token_auth0)
        logger.debug("Request ready")
        content = impl_result[0] if isinstance(impl_result, tuple) else impl_result
        if isinstance(impl_result, Response):
            return impl_result
...
async def update_user(
    ...
    if not hasattr(accounts_api_impl, 'update_user') or not callable(getattr(accounts_api_impl, 'update_user')):
        raise NotImplementedError("The /update_user endpoint is not yet implemented")

    try:
        logger.debug("Starting request")
        impl_result = await accounts_api_impl.update_user(uid, user_metadata, pool=request.app.state.pool, token_model=token_auth0)
        logger.debug("Request ready")
        content = impl_result[0] if isinstance(impl_result, tuple) else impl_result
        if isinstance(impl_result, Response):
            return impl_result
