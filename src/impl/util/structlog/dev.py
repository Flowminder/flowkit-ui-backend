...
# stolen from https://github.com/hynek/structlog/blob/f123768e820376338e5048b7c87e29db0b0b3f94/src/structlog/dev.py
# as per https://github.com/hynek/structlog/issues/388
...
class ConsoleRenderer:
    ...
    def __call__(self, logger: WrappedLogger, name: str, event_dict: EventDict) -> str:
        ...
        # add filename/line no and function
        file_name = event_dict.pop("filename", None)
        ...
        # apply colours to actual text
        event_style = ""
        if level == "debug":
            event_style = self._styles.dim
        elif level == "warning":
            event_style = self._styles.level_warn
        elif level == "error":
            event_style = self._styles.level_error
        elif level == "critical":
            event_style = self._styles.level_critical

        event = f"{event_style}{str(event)}{self._styles.reset}"
